...
class ImagesCollection final
{
	public:
		...
		RgbaFloat getColor(const std::string &layer_name, int x, int y) const;
		...
		Image *findLayer(const std::string &layer_name);
		const Image *findLayer(const std::string &layer_name) const;
		void clear();
